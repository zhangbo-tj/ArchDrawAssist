...
// 	{	vec[0] = x0; vec[1] = x1; vec[2] = x2; };
	...
	{	vec[0] = x0; vec[1] = x1; vec[2] = x2; };
	void Set( float x0,  float x1,  float x2)
	{	vec[0] = x0; vec[1] = x1; vec[2] = x2; };
	double GetFirst(){return vec[0];}
	double GetSecond(){return vec[1];}
	double GetThird(){return vec[2];}
    int Dimension() const
		{	return 3; };
    double & operator [](const int i)           // index i'th element
		{	return(vec[i]); };
	...
	{   vec[0] -= v0(0); vec[1] -= v0(1); vec[2] -= v0(2);
		return (*this); }
	const VECTOR3& operator *=(const double & v0) //*= overload
	{   vec[0] *= v0; vec[1] *= v0; vec[2] *= v0;
	return (*this); }

	bool IsExactSame(const VECTOR3 &v0, double err=1.0e-5)
	{
		double xer = fabs(vec[0] - v0.vec[0]);
		double yer = fabs(vec[1] - v0.vec[1]);
		double zer = fabs(vec[2] - v0.vec[2]);

		if(xer>err) return false;
		if(yer>err) return false;
		if(zer>err) return false;
		return true;
	}
};
